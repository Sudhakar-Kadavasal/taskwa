...
def health_restart(request: Request):
    """Manual restart: restart the WhatsApp session, keeping the pairing (no QR).
    The one button to click when the gateway shows FAILED/STOPPED and messages
    aren't going out. There is no automatic restart - this is deliberate."""
    if (r := _guard(request)):
        return r
...
def health_repair(request: Request):
    """Manual re-pair: log the device out and start fresh so a new QR appears.
    Use only when WhatsApp actually de-authorised the number (a restart returns
    a QR instead of recovering)."""
    if (r := _guard(request)):
        return r
    waha.logout_session()
    waha.start_session()
